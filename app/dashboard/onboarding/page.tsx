...
export default async function OnboardingDashboardPage() {
  ...
  if (role === "applicant") {
    const [applicationsPage, availableProperties] = await Promise.all([
      listApplicationsForApplicantPage(user, { page: 1, pageSize: 100 }),
      listPublicAvailableProperties(),
    ])

    const applications = applicationsPage.items
    const activeApplications = applications.filter((application) => application.status !== "declined")
    const approvedApplications = applications.filter(
      (application) =>
        application.status === "approved" ||
        application.status === "approved_with_guarantor" ||
        application.status === "active_tenant",
    )

    const steps: WorkflowStep[] = [
      {
        title: "Discover available homes",
        description:
          availableProperties.length > 0
            ? `${availableProperties.length} currently available listing${availableProperties.length === 1 ? "" : "s"}.`
            : "No available listings right now.",
        state: activeApplications.length > 0 ? "complete" : "active",
      },
      {
        title: "Submit pre-screening application",
        description:
          activeApplications.length > 0
            ? `${activeApplications.length} active application${activeApplications.length === 1 ? "" : "s"}.`
            : "Start your first application to begin referencing.",
        state: activeApplications.length > 0 ? "complete" : "active",
      },
      {
        title: "Track approval and documents",
        description:
          approvedApplications.length > 0
            ? `${approvedApplications.length} application${approvedApplications.length === 1 ? "" : "s"} moved to approved stages.`
            : "Waiting for landlord or reviewer decision.",
        state: approvedApplications.length > 0 ? "active" : "next",
      },
      {
        title: "Transition to tenant access",
        description: "After final approval and sign-off, your role moves from applicant to tenant.",
        state: applications.some((application) => application.status === "active_tenant") ? "active" : "next",
      },
    ]
    ...
    return (
      <div className="space-y-6">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-2 text-3xl font-bold text-slate-900">Applicant onboarding workflow</h1>
          <p className="mt-2 max-w-3xl text-sm text-slate-600">
            One place to track where you are in the renter journey from first application through tenancy activation.
          </p>
        </section>
