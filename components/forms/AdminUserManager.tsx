...
export default function AdminUserManager({ initialUsers, initialAgents, currentUserEmail }: AdminUserManagerProps) {
  ...
  function completeApplicantAccountErasure(user: AuthUser) {
    const confirmed = window.confirm(`Permanently erase ${getFullName(user)} (${user.email}) and their application data? This is irreversible and is permitted only when the Applicant has never held an active tenancy.`)
    ...
    startTransition(async () => {
      try {
        ...
        setUsers((current) => sortUsers(current.filter((candidate) => candidate.email !== user.email)))
        setFeedback({ type: "success", message: `Applicant account erased for ${getFullName(user)}.` })
      } catch (error) {
        setFeedback({ type: "error", message: error instanceof Error ? error.message : "Unable to erase applicant account." })
      } finally {
        setSavingEmail(null)
      }
    })
                  ...
                  <td className="px-4 py-4 text-right">
                    <div className="inline-flex flex-col items-end gap-2">
                      ...
                      {user.role === "applicant" && user.accountErasureRequestedAt ? (
                        <button
                          type="button"
                          onClick={() => completeApplicantAccountErasure(user)}
                          disabled={isPending && savingEmail === user.email}
                          className="rounded-md bg-rose-700 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-rose-800 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          {isPending && savingEmail === user.email ? "Erasing..." : "Complete account erasure"}
                        </button>
                      ) : null}
                    </div>
                  </td>
