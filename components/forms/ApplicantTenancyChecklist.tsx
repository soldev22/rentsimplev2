...
const verificationChecklistOptions: Array<{
  key:
    | "noIdRequired"
    | "photoIdReceived"
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Read-only documents uploaded by your landlord or agent for applicant verification.
            </p>
          </div>
          <button
            type="button"
            className="rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700"
            aria-label={isVerificationDocsOpen ? "Collapse panel" : "Expand panel"}
            onClick={() => setIsVerificationDocsOpen((current) => !current)}
          >
            <span className={`inline-block text-[2.5rem] leading-none transition-transform ${isVerificationDocsOpen ? "rotate-0" : "-rotate-90"}`}>▾</span>
          </button>
        </div>

        {isVerificationDocsOpen ? (
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {verificationChecklistOptions.map((option) => {
              const documents = (application.referencingInstruction.verificationDocuments ?? []).filter(
              ...
              return (
                <div key={option.key} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] ${documents.length > 0 ? "bg-emerald-100 text-emerald-900" : "bg-slate-200 text-slate-700"}`}>
                      {documents.length > 0 ? `${documents.length} file${documents.length > 1 ? "s" : ""}` : "No files"}
                    </span>
                  </div>
                  ...
                  {documents.length > 0 ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {documents.map((document) => (
                        <a
                          ...
                          className="inline-flex rounded-md border border-slate-300 bg-white px-3 py-1 text-xs font-semibold text-slate-800 hover:bg-slate-100"
                        >
                          {document.fileName}
                        </a>
                      ))}
                    ...
                    <p className="mt-3 text-sm text-slate-600">No document uploaded for this verification item yet.</p>
                  )}
                </div>
              )
            })}
